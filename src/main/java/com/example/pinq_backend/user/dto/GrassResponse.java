 ...
 * 기존 등급을 소급 강등시키지 않는다.
 *
 * 잔디는 '신규 학습(첫 시도)'만의 지표다 (2026-08-08 개정): 복습은 등급에 안 들어가고
 * **복습만 한 날은 칸 자체가 생기지 않는다** — 스트릭도 복습을 안 세므로 축이 일관된다.
 * 복습의 성과는 {@code graduatedTrees}(나무)·정원으로만 표현된다. 같은 날 신규 학습과
 * 복습을 함께 하면 칸은 신규 학습으로 생기고 {@code reviewed} 가 병기된다.
 *
 * @param from            잔디 시작일 (오늘 - 364일)
 * @param to              오늘
 * @param totalActiveDays 기간 내 활동일 수 (신규 학습을 시도한 날 — 복습만 한 날 제외)
