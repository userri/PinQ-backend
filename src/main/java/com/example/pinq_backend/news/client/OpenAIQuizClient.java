...
public class OpenAIQuizClient {
    ...
    private static final String CAUSAL_RULEBOOK = """
            ...
            [자본 흐름 → 환율] — 방향 혼동이 잦은 축이므로 주체를 먼저 확정할 것
            - 외국인이 국내 자산을 매수(자금 유입) → 달러 매도·원화 매수 → 원화 강세(환율 하락)
            - 외국인이 국내 자산을 매도해 본국으로 회수(자금 유출) → 원화 매도·달러 매수
              → 원화 약세(환율 상승)
            - 내국인이 해외 투자를 확대 → 원화 매도·달러 매수 → 환율 상승
            - 내국인이 해외 투자금을 국내로 환류(회수) → 달러 매도·원화 매수 → 환율 하락
            ※ '환류·회수'는 주체가 외국인이냐 내국인이냐에 따라 환율 방향이 정반대다.
              주체를 먼저 확정하지 않고 서술하면 방향이 뒤집힌다.
            ...
            [물가]
            - 물가 상승 → 화폐의 실질 구매력 감소, 실질금리(명목금리-물가상승률) 하락
            - 통화량 증가·환율 상승(수입물가) → 물가 상승 압력
            - 물가 상승 지속 → 중앙은행 기준금리 인상 압력 (긴축)""";
    ...
    private boolean verifyAnswer(GeneratedQuizDto quiz, Category category, List<String> recentQuestions,
        ...
        String verifyPrompt = """
                ...
                14. 질문에 생소한 경제 지표·전문 용어(예: 순상품교역조건지수, 근원 PCE,
                    신용 스프레드)가 한 줄 정의 없이 등장하고, 정답을 고르려면 그 용어의
                    정의 지식이 필요하면 valid는 false입니다. 정의는 질문 문장 안에서
                    주어져야 하며, 정답은 그 정의를 암기했는지가 아니라 적용·해석·결과
                    판단을 물어야 합니다.
    ...
    static String systemPrompt(Category category) {
        return """
            ...
            ## 지표 접근성 규칙 (대상 독자는 경제 입문자입니다)
            생소한 경제 지표나 전문 용어(예: 순상품교역조건지수, 근원 PCE, 신용 스프레드)를
            소재로 쓸 때는 반드시:
            1. 질문 문장 안에 그 용어의 한 줄 정의를 자연스럽게 포함하세요.
            2. 정의를 암기했는지가 아니라, 정의가 주어진 상태에서의 적용·해석·결과 판단을
               물으세요. 용어의 정의 자체를 정답으로 고르게 하는 문제는 금지합니다.
            (나쁨) "순상품교역조건지수란 무엇인가?"
            (좋음) "수출 가격 대비 수입 가격의 비율인 순상품교역조건지수가 하락했다면,
                   이것이 의미하는 바는?"
    ...
    static String categoryExamples(Category category) {
        return switch (category) {
            ...
            case INFLATION -> """
                예시 1 — 개념 비교:
                Q. 소비자물가지수(CPI)와 생산자물가지수(PPI)의 가장 큰 차이는?
                정답: CPI는 소비자가 구매하는 상품·서비스 가격을 측정하고,
                      PPI는 생산자가 출하하는 단계의 가격을 측정한다

                예시 2 — 메커니즘:
                Q. 물가가 오르는데 임금이 그대로일 때 가계의 실질 소득이 줄어드는 이유는?
                정답: 명목 임금은 같아도 같은 돈으로 살 수 있는 상품·서비스의 양이
                      줄어들어 화폐의 실질 구매력이 감소하기 때문""";
        };
    }
    ...
    private String userPrompt(String title, String content, Category category, List<String> recentQuestions) {
        ...
        if (recentQuestions != null && !recentQuestions.isEmpty()) {
            recentSection = """
                ...
                %s
                ※ 각 항목 맨 앞의 [대괄호]는 그 문항의 핵심 소재입니다.
                   같은 소재는 질문 각도를 바꿔도 중복입니다 — 다른 소재를 고르세요.
