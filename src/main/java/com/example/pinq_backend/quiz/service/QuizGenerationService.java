...
public class QuizGenerationService {
    ...
    private static final Map<Category, List<String>> CATEGORY_KEYWORDS = Map.of(
            ...
            // EXCHANGE_RATE 키워드 개편(2026-08-18): 종전 4개가 전부 '가격·시장 이름'이라 외환 마감
            // 시황을 불러왔고, 그건 SKIP 기준 "사건의 단순 수치(금액)"에 정확히 걸린다. 8/17~8/18
            // 실측 SKIP율: 원달러 환율 65% / 외환시장 69% / 달러 환율 78% / "환율" 단독 100%(14건 전량).
            // 같은 이틀 STOCK "코스피"가 36% 였다. 사흘 연속 정기 회차 실패의 원인이 프롬프트가 아니라
            // 여기임을 손실 집계로 확인했다(LLM_SKIP 이 EXCHANGE_RATE 탈락의 85%).
            //  - "환율" 단독 제거: 전량 SKIP 이면서 무관 기사 유입원이다(건국대 취업 지원·하나투어
            //    임원퇴직금이 이 검색으로 들어왔다). 최후 폴백이라 제일 급할 때 도는데 수확이 0 이다.
            //  - 순서는 실측 SKIP율 오름차순.
            //  - 뒤 3개는 개념·제도 키워드다. STOCK 이 2026-07-18 에 쓴 처방과 같다 — 시황이 도배하는
            //    날에도 평시 기사를 반환해 풀 퇴화를 완충한다.
            //  - 방향성 키워드("환율 상승")는 제외. 사이클이 바뀌면 빈 검색이 된다(INTEREST_RATE 선례).
            // 직접 측정(2026-08-18): 네이버 API Hub 를 키워드별로 직접 호출해 상위 10건 제목을
            // 시황/무관/개념·제도로 육안 분류했다. LLM 은 태우지 않았다. 상세는 quality-audit-log.md.
            //  - "환헤지" 제거: 반환 10건 중 제목에 '환헤지'가 든 게 0건이다. 일본 국채금리·펀드
            //    순자산 기사만 왔다 — 매칭 자체가 안 된다. 운영 SKIP율로는 볼 수 없었다(뒤쪽 키워드라
            //    표본이 안 쌓인다).
            //  - "달러 환율" 제거: 무관 6/10 으로 "원달러 환율"의 열화판이다(실측 SKIP 78% 와 일치).
            //  - "원화 국제화"(개념 8/10)·"통화스와프"(6/10) 투입. 개념 밀도가 현행 어느 키워드보다 높다.
            Category.EXCHANGE_RATE, List.of("원달러 환율", "외환시장", "원화 국제화",
                                            "외환보유액", "통화스와프", "기축통화"),
            // STOCK 폴백 보강(2026-07-18): 급변동일엔 "코스피/주가/증시"가 시황·급락
            // 기사로 도배돼 전량 SKIP → 4/5 발행 사고. 개념성 키워드는 급변동일에도
            // 평시 기사를 반환하므로 뒤에 배치해 풀 퇴화를 완충한다.
            // 직접 측정(2026-08-18): "배당"(관련 3/10 — 문화마을·사기수사·대만 총통)·"공모주"(2/10 —
            // 축구감독·편의점)·"ETF"(관련 5/10, 대부분 비트코인 ETF)·"주가"(5/10)를 빼고, 제도·거버넌스
            // 키워드로 갈았다. "코스닥 지수"는 후보였으나 10/10 이 마감 시황이라 탈락시켰다.
            Category.STOCK,         List.of("코스피", "증시", "주주환원", "자사주 소각", "상장폐지", "공매도"),
            // 직접 측정(2026-08-18): "부동산" 단독은 10건 중 6건이 정치 기사(용산공원·당대표)였다.
            // 제도 키워드로 대체 — 임대차보호법 8/10(보증금 반환·계약 해지 기준), DSR 규제 9/10
            // (총량규제·주담대 한도), 종합부동산세 8/10(세제개편·보유세).
            // "재건축 규제"는 관련 9/10 이지만 전량이 정치인 발언 기사라 탈락시켰다.
            Category.REAL_ESTATE,   List.of("주택담보대출", "아파트 매매", "전세",
                                            "임대차보호법", "DSR 규제", "종합부동산세"),
            // INFLATION(2026-07-11 신설): 생활 밀착 소재로 뉴스 풀이 매일 풍부.
            // 직접 측정(2026-08-18): "물가 상승"(관련 3/10)·"물가" 단독(3/10)은 지역·정치 기사를
            // 끌어온다. 대체 후보 중 "소비자물가지수"(1/10)·"생산자물가"(0/10)는 오히려 전량이
            // 환율·국채 시황이었다 — 좁은 지표명이 그날 거시 헤드라인에 흡수된다.
            // 개념 키워드가 유일하게 살아남았다: 디플레이션 6/10, 물가안정목표 5/10.
            Category.INFLATION,     List.of("소비자물가", "인플레이션", "디플레이션", "물가안정목표")
    );
