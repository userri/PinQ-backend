...
public class QuizGenerationService {
    ...
    private static final Map<Category, List<String>> CATEGORY_KEYWORDS = Map.of(
            ...
            //    날에도 평시 기사를 반환해 풀 퇴화를 완충한다. ⚠️ 이 셋은 아직 돌아본 적이 없어
            //    실측이 없다. `searchKeyword` 별 SKIP율로 판정한다(scripts/keyword-stats.py).
            //    기존 넷보다 나쁘면 되돌린다.
            //  - 방향성 키워드("환율 상승")는 제외. 사이클이 바뀌면 빈 검색이 된다(INTEREST_RATE 선례).
            Category.EXCHANGE_RATE, List.of("원달러 환율", "외환시장", "달러 환율",
                                            "환헤지", "외환보유액", "기축통화"),
