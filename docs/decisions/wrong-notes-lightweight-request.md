...
> 2026-07-28 갱신: 프론트 오답노트 UI 가 인라인 아코디언(카드 펼치기) → **목록·상세 화면 분리**로
> 바뀌었다. **API 계약은 그대로**이며(목록=요약, 상세=단건 조회), 아래 본문의 "펼칠 때" 표현은
> "상세 화면 진입 시"로 읽으면 된다. 용어를 현행 UI 기준으로 갱신해 둔다.
...
- 목록 화면(목록 행)에서 실제로 쓰는 건 **질문 한 줄 + 카테고리 + 상태 뱃지**뿐이다.
- 무거운 필드(`choices`, `explanation`, `keyword`, `article`)는 **상세 화면에서만** 필요하다.
...
### ② 단건 상세 조회 엔드포인트 신설 (상세 화면 진입 시 호출)
```
...
- 상세는 진입 시 1건만 왕복 → 체감 지연 거의 없음(캐시 가능).
...
3. `AttemptItemCard` — 상세 진입 시 지연 로드(로딩/에러 상태), 1회 로드 후 캐시
